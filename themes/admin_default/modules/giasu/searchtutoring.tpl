		...
		<table class="table table-striped table-bordered table-hover">
			<thead>
				<tr>
					<th>{LANG.weight}</th>
					<th>{LANG.namestu}</th>
					<th>{LANG.numberstu}</th>
					<th>{LANG.teaching}</th>
					<th>{LANG.sessionnum}</th>
					<th>{LANG.training}</th>
					<th>{LANG.email}</th>
			...
			<tbody>
				...
				<tr>
					...
					<td> {VIEW.namestu} </td>
					<td> {VIEW.numberstu} </td>
					<td> {VIEW.teaching} </td>
					<td> {VIEW.sessionnum} </td>
					<td> {VIEW.training} </td>
					<td> {VIEW.email} </td>
	...
	<div class="table-responsive">
		<table class="table table-striped table-bordered table-hover">
			<tbody>
				<tr>
					<td> {LANG.namestu} </td>
					<td><input class="form-control" type="text" name="namestu" value="{ROW.namestu}" required="required" oninvalid="setCustomValidity( nv_required )" oninput="setCustomValidity('')" /></td>
				</tr>
				<tr>
					<td> {LANG.alias} </td>
				...
				<tr>
					<td> {LANG.numberstu} </td>
					<td><input class="form-control" type="text" name="numberstu" value="{ROW.numberstu}" pattern="^[0-9]*$"  oninvalid="setCustomValidity( nv_digits )" oninput="setCustomValidity('')" /></td>
				</tr>
				<tr>
					<td> {LANG.teaching} </td>
					<td><input class="form-control" type="text" name="teaching" value="{ROW.teaching}" required="required" oninvalid="setCustomValidity( nv_required )" oninput="setCustomValidity('')" /></td>
				</tr>
				<tr>
					<td> {LANG.sessionnum} </td>
					<td><input class="form-control" type="text" name="sessionnum" value="{ROW.sessionnum}" pattern="^[0-9]*$"  oninvalid="setCustomValidity( nv_digits )" oninput="setCustomValidity('')" /></td>
				</tr>
				<tr>
					<td> {LANG.training} </td>
					<td><input class="form-control" type="text" name="training" value="{ROW.training}" required="required" oninvalid="setCustomValidity( nv_required )" oninput="setCustomValidity('')" /></td>
				</tr>
				<tr>
					<td> {LANG.email} </td>
...
//<![CDATA[
	function nv_get_alias(id) {
		var title = strip_tags( $("[name='namestu']").val() );
		if (title != '') {
			$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=searchtutoring&nocache=' + new Date().getTime(), 'get_alias_title=' + encodeURIComponent(title), function(res) {
	...
	function nv_change_weight(id) {
		...
		$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=searchtutoring&nocache=' + new Date().getTime(), 'ajax_action=1&id=' + id + '&new_vid=' + new_vid, function(res) {
			var r_split = res.split('_');
			if (r_split[0] != 'OK') {
				alert(nv_is_change_act_confirm[2]);
			}
			clearTimeout(nv_timer);
			window.location.href = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=searchtutoring';
			return;
		});
		return;
	}
